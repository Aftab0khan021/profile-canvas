    ...
    >
      ...
      {blogs.length === 0 ? (
        ...
        <div className="space-y-3">
          {blogs.map((blog) => (
            <div key={blog.id} className="bento-card">
              <div className="flex items-start justify-between mb-2">
                <div className="space-y-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="font-semibold text-sm truncate">{blog.title}</p>
                    <span className={`px-2 py-0.5 rounded-full text-[11px] font-semibold shrink-0 ${blog.status === 'published' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-muted text-muted-foreground'}`}>
                      {blog.status}
                    </span>
                  </div>
                ...
                <div className="flex gap-0.5 shrink-0">
                  ...
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditDialog(blog)}>
                    <Pencil className="h-3.5 w-3.5" />
                  </Button>
                  <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => handleDelete(blog.id)}>
                    <Trash2 className="h-3.5 w-3.5 text-destructive" />
                  </Button>
                </div>
              </div>
              {blog.content && (
                <p className="text-xs text-muted-foreground line-clamp-2">{blog.content}</p>
              )}
            </div>
          ))}
        </div>
